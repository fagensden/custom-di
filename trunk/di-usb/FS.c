...
s32 ISFS_ReadDir(const char *filepath, char *name_list, u32 *num)
{
	if(name_list == NULL)
	{
		vect *v = (vect*)halloca(sizeof(vect) * 2, 0x40);
		...
		vect *v = (vect*)halloca( sizeof(vect)*4, 0x40);
...
{
	u8 *data = (u8*)malloca(0x40, 32);	
	s32 r = IOS_Ioctl(FSHandle, ISFS_GET_DI_PATH, NULL, 0, (void*)data, 0x40);	
	memcpy(filepath, data, 0x40);
	return r;	
}
