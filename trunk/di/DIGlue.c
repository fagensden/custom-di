...
// when this is turned on, the system fakes uneek mode, even when used with sneek
// it allows operation without harddisk as used in my poor man's sneaky backup thing.
// but is incompatibel with dml as that one will only work in sneek mode
// besides that it needs a working harddisk in normal mode,
...
#else
s32 ISFS_Support_SD_DI( void )
{
	s32 fd = IOS_Open( "/dev/fs", 0 );
	s32 ret = IOS_Ioctl( fd, ISFS_SUPPORT_SD_DI, NULL, 0, NULL, 0 );
	IOS_Close( fd );
...
#else
	if( ISFS_Support_SD_DI() == FS_ENOENT2 )
#endif
	{
		FSMode = SNEEK;
