...
s32 ES_GetUID( u64 *TitleID, u16 *UID )
{
	...
	*UID = 0xdead;
	...
	{																		 
//obcd
//dirty conversion to lowercase
//of second title id char
//so making that one invisible
//ensures that it will always be visible

		if( (uid[i].TitleID & 0xFFFFFFFFDFFFFFFFULL) == *TitleID )
		{																	
			*UID = uid[i].GroupID;							 
			break;
		}
...
//Find boot index
	...
	{
		if( *TitleID >= 0x0000000100000099LL )
		{
