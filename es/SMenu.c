...
void SMenuDraw( void )
{
	...
	for( i=0; i < MAX_FB; i++)
	{
		...
		if( MenuType != 3 )
		{
			...
			{
				if(LoadDI == true)
					PrintFormat( FB[i], MENU_POS_X, 20, "ONEEKu+DI v4 %s Games:%d Region:%s", __DATE__, *GameCount, RegionStr[DICfg->Region] );
				else
					PrintFormat( FB[i], MENU_POS_X, 20, "ONEEKu v4 %s",__DATE__);					
			} 
			else 
			{
				if(LoadDI == true)
					PrintFormat( FB[i], MENU_POS_X, 20, "ONEEKs+DI v4 %s Games:%d Region:%s", __DATE__, *GameCount, RegionStr[DICfg->Region] );
				else
					PrintFormat( FB[i], MENU_POS_X, 20, "ONEEKs v4 %s",__DATE__);					
			}
		}
